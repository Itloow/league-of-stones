...
export default function Game() {
    ...
    }, [matchData, myPlayer, enemyPlayer, matchFinished]);
    ...
    return (
        <div className={styles.gamePage}>
            ...
            <div className={styles.mobileGameHeader}>
                ...
                <button className={styles.mobileProfileBtn} onClick={() => router.push('/profil')}>
                    {name || 'Profil'}
                </button>
            </div>

            {error && !matchFinished && <div className={styles.errorMsg}>{error}</div>}
            ...
            <div className={styles.boardContainer}>
                <div className={styles.enemySection}>
                    <div className={styles.playerInfo}>
                        <div className={styles.avatarEnemy}>
                            <img
                                src="https://ddragon.leagueoflegends.com/cdn/img/champion/loading/Garen_0.jpg"
                                alt="adversaire"
                    ...
                    <div className={styles.slotsRow}>
                        ...
                        {enemyPlayer.board &&
                            Array.from({ length: 5 - (enemyPlayer.board.length || 0) }).map((_, i) => (
                ...
                <div className={styles.mySection}>
                    <div className={styles.slotsRow}>
                        ...
                        {myPlayer.board &&
                            Array.from({ length: 5 - (myPlayer.board.length || 0) }).map((_, i) => (
                            ...
                            return (
                                ...
                                >
                                    <img
                                        src={getCardImage(card.key)}
                                        alt={card.name}
                                        className={`${styles.handCardImg} ${isMyTurn && (!myPlayer.board || myPlayer.board.length < 5) ? styles.clickable : ''}`}
                                        onClick={() => isMyTurn && handlePlayCard(card.key)}
                                        style={{
                                            cursor: isMyTurn && (!myPlayer.board || myPlayer.board.length < 5) ? 'pointer' : 'default',
                                        }}
                                    />
                                    <div className={styles.handCardStats}>
                                        <span className={styles.atkStat}>⚔️ {card.info?.attack || '?'}</span>
